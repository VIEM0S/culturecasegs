...
const Reports = memo(function Reports({ data }) {
    ...
    return Object.entries(totals)
      .map(([id, v]) => {
        ...
        return {
          id,
          ...v,
          name: p ? `${p.model} — ${p.design}` : "—",
          shortName: p ? p.design : "—",
        };
      })
  ...
  const totalRevenue = useMemo(
    () => sales.reduce((s, v) => s + (v.totalAfterDiscount ?? v.total), 0),
    [sales],
  );
  const totalDiscounts = useMemo(
    () => sales.reduce((s, v) => s + (v.discountAmount || 0), 0),
    [sales],
  );
  const totalUnits = useMemo(
    () => sales.reduce((s, v) => s + v.qty, 0),
    [sales],
  );
  ...
  const topChartData = useMemo(
    () =>
      topProducts.slice(0, 7).map((p) => ({
        name:
          p.shortName.length > 14
            ? p.shortName.slice(0, 13) + "…"
            : p.shortName,
        fullName: p.name,
        qty: p.qty,
        revenue: p.revenue,
