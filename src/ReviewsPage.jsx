...
import { db } from "./firebase"; // ajuster le chemin selon ton projet
...
function StatusBadge({ status }) {
  const map = {
    pending:   { label: "En attente", bg: "#FFF8E6", color: "#8B6300", border: "#F5C842" },
    published: { label: "Publié",     bg: "#EDF7F1", color: "#1E5C35", border: "#52B37A" },
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 13, color: "#1E0900" }}>{nom}</div>
            <div style={{ fontSize: 11, color: "#888" }}>{loc || "Bamako, Mali"}</div>
          </div>
        </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        <Stars n={stars || 5} />
        <span style={{ fontSize: 10, color: "#AAA" }}>{formatDate(createdAt)}</span>
      </div>

      {/* Texte */}
      <p style={{
        fontSize: 13.5, color: "#333", lineHeight: 1.7,
        margin: 0, fontStyle: "italic",
        borderLeft: "3px solid #E8A020", paddingLeft: 12,
      }}>
        « {txt} »
      </p>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4 }}>
        ...
        <button
          ...
          style={{
            ...
            border: "1.5px solid #FFCDD2", cursor: "pointer",
...
export default function ReviewsPage() {
  ...
  return (
    <div style={{ padding: "1.5rem 2rem", maxWidth: 900, margin: "0 auto", fontFamily: "'DM Sans', sans-serif" }}>

      {/* En-tête */}
      <div style={{ marginBottom: "1.5rem" }}>
        <h1 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 32, letterSpacing: 2, color: "#1E0900", margin: 0 }}>
          AVIS CLIENTS
        </h1>
        <p style={{ fontSize: 13, color: "#888", marginTop: 4 }}>
          Modérez les avis soumis depuis le site. Seuls les avis publiés apparaissent sur l'accueil.
        </p>
      </div>
      ...
      {error && (
        <div style={{
          ...
          padding: "10px 14px", marginBottom: 16, fontSize: 13, color: "#C62828",
          display: "flex", justifyContent: "space-between", alignItems: "center",
        }}>
          {error}
          <button onClick={() => setError(null)} style={{ background: "none", border: "none", cursor: "pointer", color: "#C62828", fontWeight: 700 }}>✕</button>
        </div>
      )}

      {/* Onglets */}
      <div style={{ display: "flex", gap: 8, marginBottom: "1.5rem", borderBottom: "2px solid #EEE", paddingBottom: 0 }}>
        {TABS.map(t => (
          <button
            key={t.key}
