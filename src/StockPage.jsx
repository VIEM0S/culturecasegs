...
function StockPage({ data, onMove, isViewer = false }) {
  ...
  const reasons = { in: ["Approvisionnement", "Retour client", "Correction"], out: ["Vente", "Échantillon", "Casse", "Correction"] };
                      ...
                      {designsForModel.map(p => {
                        ...
                        return (
                          <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "5px 8px", borderRadius: 7, background: checked ? "rgba(124,58,237,0.1)" : "transparent", border: `1px solid ${checked ? "var(--accent2)" : "var(--border)"}`, transition: "all 0.12s" }}>
                            ...
                            {(() => { const img = getProductImageUrl(p, designs); return img ? <img src={img} alt={p.design} loading="lazy" style={{ width: 28, height: 28, borderRadius: 5, objectFit: "cover", flexShrink: 0 }} onError={e => { e.target.style.display = "none"; }} /> : null; })()}
