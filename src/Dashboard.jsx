...
const Dashboard = memo(function Dashboard({ data, isViewer = false }) {
  ...
  const chart30 = useMemo(() => {
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      const revenue  = daySales.reduce((s, v) => s + (v.totalAfterDiscount ?? v.total), 0);
  ...
  const top5 = useMemo(() => {
    ...
    return Object.entries(totals)
      .map(([id, v]) => {
        ...
        return { id, ...v, name: p ? `${p.design}` : "—", fullName: p ? `${p.model} — ${p.design}` : "—" };
      })
  ...
  const recentOps = useMemo(() => {
    ...
    const groupedSales = Array.from(saleGroups.values()).map((group) => {
      ...
      return { ...first, _type: "sale", _group: group, _groupTotal: groupTotal, _totalQty: totalQty, _isMulti: group.length > 1 };
    });
    return [
      ...groupedSales,
  ...
  const trend = useMemo(() => {
    ...
    const lastWeek = sales.filter((s) => new Date(s.date) >= twoWeeksAgo && new Date(s.date) < weekAgo);
    const revThis  = thisWeek.reduce((s, v) => s + (v.totalAfterDiscount ?? v.total), 0);
    const revLast  = lastWeek.reduce((s, v) => s + (v.totalAfterDiscount ?? v.total), 0);
    const diff     = revLast > 0 ? Math.round(((revThis - revLast) / revLast) * 100) : null;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <div className="grid-2">
        {!isViewer && (
          <div className="card">
            ...
            <p style={{ fontSize: 28, fontWeight: 800, color: "var(--success)", lineHeight: 1.1 }}>
            ...
            {trend.diff !== null && (
              <div style={{
                ...
                background: trend.diff >= 0 ? "rgba(16,185,129,0.1)" : "rgba(239,68,68,0.1)",
                color:      trend.diff >= 0 ? "var(--success)"       : "var(--danger)",
              }}>
                {trend.diff >= 0 ? "▲" : "▼"} {Math.abs(trend.diff)}% vs semaine dernière
              </div>
            )}
          </div>
        )}
      ...
      {!isViewer && sales.length > 0 && (
        <div className="card">
          <p className="section-label" style={{ marginBottom: 16 }}>Chiffre d'affaires — 30 derniers jours</p>
          <ResponsiveContainer width="100%" height={180}>
            <AreaChart data={chart30} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
              <defs>
              ...
              <XAxis
                dataKey="label" tick={{ fontSize: 10, fill: "var(--text2)" }}
              ...
              <Area
                ...
                fill="url(#gradCA)" dot={false} activeDot={{ r: 4 }}
              />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      ...
      <div className="grid-2">
        ...
        {sales.length > 0 && (
          <div className="card">
            <p className="section-label" style={{ marginBottom: 14 }}>Top 5 produits vendus</p>
            {top5.length === 0 && (
              <p style={{ fontSize: 13, color: "var(--text2)" }}>Aucune vente enregistrée</p>
            )}
            {top5.map((p, i) => (
              <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                ...
                {/* Mini barre */}
                <div style={{ width: 50, height: 4, borderRadius: 2, background: "var(--bg3)", flexShrink: 0 }}>
                  <div style={{
